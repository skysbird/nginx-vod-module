...
/*
#define video_FILTER_DEBUG
#define video_FILTER_DEBUG_PATH "/tmp/"
*/
...
} video_filter_encoder_t;

typedef struct
{
	video_decoder_state_t decoder;
	AVFilterContext *buffer_src;
	bool_t buffersrc_flushed;
} video_filter_source_t;

typedef struct
{
	AVFilterContext *buffer_sink;
	video_filter_encoder_t* encoder;
	void* encoder_context;
	vod_array_t frames_array;
} video_filter_sink_t;

// constants
static video_filter_encoder_t libav_encoder = {
	video_ENCODER_INPUT_SAMPLE_FORMAT,
	video_encoder_free,
	video_encoder_get_frame_size,
	video_encoder_write_frame,
	video_encoder_flush,
	video_encoder_update_media_info,
};

static video_filter_encoder_t volume_map_encoder = {
...
typedef struct {
	...
	int cache_slot_id;
#endif
} video_filter_init_context_t;

static vod_status_t 
video_filter_walk_filters_prepare_init(
	...
	uint32_t speed_denom)
{
	...
	if (media_clip_is_source(clip->type))
	{
		...
		for (cur_track = source->track_array.first_track; cur_track < source->track_array.last_track; cur_track++)
		{
			...
			if (video_track != NULL)
			{
				vod_log_error(VOD_LOG_ERR, state->request_context->log, 0,
					"video_filter_walk_filters_prepare_init: more than one video track per source - unsupported");
		...
		if (video_track == NULL || video_track->frame_count == 0)
		{
			*clip_ptr = NULL;
			return VOD_OK;
		}
		...
		cur_frame_count = ((uint64_t)video_track->frame_count * speed_denom) / speed_num;
...
vod_status_t
video_filter_alloc_memory_frame(
	request_context_t* request_context,
	vod_array_t* frames_array,
	size_t size,
	input_frame_t** result)
{
	...
	if (cur_frame == NULL)
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"video_filter_alloc_memory_frame: vod_array_push failed");
	...
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"video_filter_alloc_memory_frame: vod_alloc failed");
		return VOD_ALLOC_FAILED;
	}
...
typedef struct {
	...
	// filter end points
	video_filter_sink_t sink;
	video_filter_source_t* sources;
	video_filter_source_t* sources_end;
...
void 
video_filter_process_init(vod_log_t* log)
{
	...
	if (buffersrc_filter == NULL)
	{
		vod_log_error(VOD_LOG_WARN, log, 0,
			"video_filter_process_init: failed to get buffer source filter, video filtering is disabled");
		return;
	}
	...
	{
		vod_log_error(VOD_LOG_WARN, log, 0,
			"video_filter_process_init: failed to get buffer sink filter, video filtering is disabled");
		return;
	}
...
static vod_status_t
video_filter_init_source(
	request_context_t* request_context,
	AVFilterGraph *filter_graph,
	const u_char* source_name,
	video_filter_source_t* source, 
	AVFilterInOut** outputs)
{
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_init_source: avfilter_inout_alloc failed");
		return VOD_ALLOC_FAILED;
	}
	...
	*outputs = output_link;
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_init_source: av_strdup failed");
		return VOD_ALLOC_FAILED;
	}
	...
	AVFilterInOut** inputs)
{
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_init_sink: avfilter_graph_create_filter failed %d", avrc);
		return VOD_ALLOC_FAILED;
	}
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_init_sink: av_opt_set_int_list(sample formats) failed %d", avrc);
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_init_sink: avfilter_inout_alloc failed");
		return VOD_ALLOC_FAILED;
	}
...
video_filter_init_sources_and_graph_desc(video_filter_init_context_t* state, media_clip_t* clip, media_track_t *output_track)
{
	...
	if (media_clip_is_source(clip->type))
	{
		...
		return video_filter_init_source(
			state->request_context,
			state->filter_graph,
			filter_name,
			cur_source,
...
video_filter_alloc_state(
	...
	void** result)
{
	video_filter_init_context_t init_context;
	u_char filter_name[VOD_INT32_LEN + 1];
	video_encoder_params_t encoder_params;
	video_filter_state_t* state;
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_alloc_state: unexpected - no sources found");
		return VOD_UNEXPECTED;
	}
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_alloc_state: module failed to initialize successfully");
		return VOD_UNEXPECTED;
	}
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_alloc_state: expected output frame count %uD too big", init_context.output_frame_count);
		return VOD_BAD_REQUEST;
	}
	...
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"video_filter_alloc_state: vod_alloc failed (1)");
		return VOD_ALLOC_FAILED;
	}
	...
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"video_filter_alloc_state: vod_pool_cleanup_add failed");
		return VOD_ALLOC_FAILED;
	}

	cln->handler = video_filter_free_state;
	cln->data = state;
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_alloc_state: avfilter_graph_alloc failed");
		return VOD_ALLOC_FAILED;
	}
	...
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"video_filter_alloc_state: vod_alloc failed (2)");
		return VOD_ALLOC_FAILED;
	}
	...
	*init_context.graph_desc_pos = '\0';
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_alloc_state: unexpected buffer sink time base %d/%d",
...
// 		encoder_params.bitrate = output_track->media_info.bitrate;
	...
	{
		vod_log_error(VOD_LOG_ERR, request_context->log, 0,
			"video_filter_alloc_state: av_frame_alloc failed");
		rc = VOD_ALLOC_FAILED;
		goto end;
	}
	...
	{
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0,
			"video_filter_alloc_state: vod_array_init failed");
		rc = VOD_ALLOC_FAILED;
		goto end;
	}
...
void
video_filter_free_state(void* context)
{
	video_filter_state_t* state = (video_filter_state_t*)context;
	video_filter_source_t* sources_cur;

	for (sources_cur = state->sources; sources_cur < state->sources_end; sources_cur++)
	{
		video_decoder_free(&sources_cur->decoder);
	}
...
static vod_status_t 
video_filter_update_track(video_filter_state_t* state)
{
	...
	{
		rc = codec_config_get_video_codec_name(state->request_context, &output->media_info);
		if (rc != VOD_OK)
		{
			return rc;
		}
...
#ifdef video_FILTER_DEBUG
static void
video_filter_append_debug_data(const char* source, const char* extension, const void* buffer, size_t size)
{
	char file_path[256];
	FILE* fp;

	sprintf(file_path, "%s%s.%s", video_FILTER_DEBUG_PATH, source, extension);
...
#endif // video_FILTER_DEBUG

static vod_status_t
video_filter_read_filter_sink(video_filter_state_t* state)
{
	vod_status_t rc;
	int avrc;
#ifdef video_FILTER_DEBUG
	...
	for (;;)
	{
		...
		{
			vod_log_error(VOD_LOG_ERR, state->request_context->log, 0,
				"video_filter_read_filter_sink: av_buffersink_get_frame_flags failed %d", avrc);
			return VOD_UNEXPECTED;
		}
		...
		data_size = av_samples_get_buffer_size(
			...
			1);
		video_filter_append_debug_data("sink", "pcm", state->filtered_frame->data[0], data_size);
#endif // video_FILTER_DEBUG
...
static vod_status_t 
video_filter_process_frame(video_filter_state_t* state, AVFrame* frame)
{
	video_filter_source_t* source = state->cur_source;
	int avrc;
	...
	data_size = av_samples_get_buffer_size(
		...
		frame->format,
		1);
	video_filter_append_debug_data(source->buffer_src->name, "pcm", frame->data[0], data_size);
#endif // video_FILTER_DEBUG
	...
	{
		vod_log_error(VOD_LOG_ERR, state->request_context->log, 0,
			"video_filter_process_frame: av_buffersrc_add_frame_flags failed %d", avrc);
		return VOD_ALLOC_FAILED;
	}

	return video_filter_read_filter_sink(state);
}

static vod_status_t
video_filter_choose_source(video_filter_state_t* state)
{
	video_filter_source_t* sources_cur;
	video_filter_source_t* best_source;
	...
	for (;;)
	{
		...
		rc = video_filter_read_filter_sink(state);
		if (rc != VOD_OK)
		{
			return rc;
		}
	...
	if (ret != AVERROR(EAGAIN))
	{
		...
		vod_log_error(VOD_LOG_ERR, state->request_context->log, 0,
			"video_filter_choose_source: avfilter_graph_request_oldest failed %d", ret);
		return VOD_UNEXPECTED;
	}
	...
	for (sources_cur = state->sources; sources_cur < state->sources_end; sources_cur++)
	{
		if (!video_decoder_has_frame(&sources_cur->decoder))
		{
			if (!sources_cur->buffersrc_flushed)
			{
				...
				{
					vod_log_error(VOD_LOG_ERR, state->request_context->log, 0,
						"video_filter_choose_source: av_buffersrc_add_frame_flags failed %d", avrc);
					return VOD_ALLOC_FAILED;
				}
...
vod_status_t
video_filter_process(void* context)
{
	video_filter_state_t* state = context;
	vod_status_t rc;
	AVFrame* frame;
	...
	for (;;)
	{
		...
		{
			rc = video_filter_choose_source(state);
			if (rc == VOD_NOT_FOUND)
			{
				// done
		...
		rc = video_decoder_get_frame(&state->cur_source->decoder, &frame);
		if (rc != VOD_OK)
		{
			return rc;
		}
		...
		// process the frame
		rc = video_filter_process_frame(state, frame);
		if (rc != VOD_OK)
		{
			return rc;
		}
	...
	void** result)
{
	...
	vod_log_error(VOD_LOG_ERR, request_context->log, 0,
		"video_filter_alloc_state: video filtering not supported, recompile with avcodec/avfilter to enable it");
	return VOD_UNEXPECTED;
}
